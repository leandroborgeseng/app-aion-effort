// scripts/importInvestmentsFromFile.ts
// Script para importar investimentos do arquivo investments-import.json para o banco de dados
...
async function importInvestments() {
  try {
    console.log('🚀 Iniciando importação de investimentos do arquivo investments-import.json...\n');

    // Ler investimentos do arquivo
    const importFile = path.join(process.cwd(), 'investments-import.json');
    console.log(`📖 Lendo arquivo: ${importFile}`);
    
    const fileContent = await fs.readFile(importFile, 'utf-8');
    const investments = JSON.parse(fileContent);
    
    if (!Array.isArray(investments)) {
      throw new Error('O arquivo de investimentos não contém um array válido');
    }

    console.log(`✅ Encontrados ${investments.length} investimentos no arquivo\n`);
    ...
    console.log(`📊 Encontrados ${existingInvestments.length} investimentos já existentes no banco\n`);
    ...
    for (const inv of investments) {
      try {
        ...
        // Mapear setor para ID
        const sectorId = mapSectorToId(inv.setor);

        // Converter valorEstimado para número (usar 0 se for null, pois o schema não permite null)
        let valorEstimado = 0;
        if (inv.valorEstimado !== null && inv.valorEstimado !== undefined) {
          if (typeof inv.valorEstimado === 'string') {
            const parsed = parseFloat(inv.valorEstimado.replace(/[^\d.,-]/g, '').replace(',', '.'));
            if (!isNaN(parsed)) {
              valorEstimado = parsed;
            }
          } else {
            const num = Number(inv.valorEstimado);
            if (!isNaN(num)) {
              valorEstimado = num;
            }
          }
        ...
        // Converter dataPrevista se existir
