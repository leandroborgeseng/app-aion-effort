...
// POST /api/ecm/rounds/:id/summary - Gerar resumo inteligente da ronda com IA
rounds.post('/:id/summary', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    ...
    if (USE_MOCK) {
      ...
      const { generateRoundSummary } = await import('../services/aiSummaryService');
      const summary = await generateRoundSummary(round);
      res.json(summary);
    } else {
      const prismaClient = await getPrisma();
      if (!prismaClient) {
      ...
      // Buscar ronda com relacionamentos
      const round = await prismaClient.sectorRound.findUnique({
        where: { id },
        include: {
          investments: {
      ...
      // Buscar detalhes das OS vinculadas
      let osDetails: any[] = [];
      if (round.osIds) {
        try {
          const osIdsArray = JSON.parse(round.osIds);
          if (Array.isArray(osIdsArray) && osIdsArray.length > 0) {
            const { dataSource } = await import('../adapters/dataSource');
            let osData = await dataSource.osResumida({
              tipoManutencao: 'Todos',
              periodo: 'MesCorrente',
            ...
            osData = await filterOSByWorkshop(osData);
            osDetails = osData.filter((os: any) => osIdsArray.includes(os.CodigoSerialOS));
          }
        } catch (e) {
          console.warn('[rounds:POST/:id/summary] Erro ao buscar detalhes das OS:', e);
        }
      }

      // Buscar detalhes das purchase requests vinculadas
      let purchaseRequests: any[] = [];
      if (round.purchaseRequestIds) {
        try {
          const prIdsArray = JSON.parse(round.purchaseRequestIds);
          if (Array.isArray(prIdsArray) && prIdsArray.length > 0) {
            purchaseRequests = await prismaClient.purchaseRequest.findMany({
              where: { id: { in: prIdsArray } },
              select: {
                id: true,
                description: true,
                status: true,
          ...
          console.warn('[rounds:POST/:id/summary] Erro ao buscar purchase requests:', e);
        }
      }
      ...
      const roundData = {
        ...
        weekStart: round.weekStart.toISOString(),
        responsibleName: round.responsibleName || undefined,
        openOsCount: round.openOsCount,
        closedOsCount: round.closedOsCount,
        notes: round.notes || undefined,
      };

      const investments = round.investments.map((inv) => ({
        id: inv.id,
        titulo: inv.titulo,
        categoria: undefined,
        valorEstimado: inv.valorEstimado ? Number(inv.valorEstimado) : undefined,
        prioridade: undefined,
      }));

      const { generateRoundSummary } = await import('../services/aiSummaryService');
      const summary = await generateRoundSummary(roundData, osDetails, purchaseRequests, investments);

      res.json(summary);
    }
  } catch (e: any) {
    console.error('[rounds:POST/:id/summary] Erro:', e);
    res.status(500).json({ error: true, message: e?.message || 'Erro ao gerar resumo' });
  }
});
